Bugs found in Roc via fuzzing:
 ...
 - `Dict.insert` reallocates the entry list on every overwrite of an existing key, even when the dictionary is uniquely owned and pre-sized. The absent-key path is free; only the `Found` branch copies. Found by allocation assertions, not by any content property -- a copying insert still returns the right answer. Red test: `trophy-case/repros/dictInsertOverwrite.roc`
