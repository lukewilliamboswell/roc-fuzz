Bugs found in Roc via fuzzing:
 ...
 - `Dict.insert` reallocated the entry list on every overwrite of an existing key, even when the dictionary was uniquely owned and pre-sized. The absent-key path was free; only the `Found` branch copied. Found by allocation assertions, not by any content property -- a copying insert still returns the right answer. Fixed for primitive keys and values by roc `046b7daa26` ("Preserve Dict uniqueness across projected field takes"), then regressed in `nightly-2026-09-05-b195f5b`; red test: `trophy-case/repros/dictInsertOverwrite.roc`.
 - The same overwrite also allocated once per call when the keys and values were refcounted (`Dict(Str, List(U8))` with keys padded past the 24-byte inline threshold), scaling linearly with the number of overwrites. Fixed by roc `92a663ecb8` ("Preserve Dict uniqueness for refcounted overwrites"), then regressed in `nightly-2026-09-05-b195f5b`; red test: `trophy-case/repros/dictInsertOverwriteRefcounted.roc`.
 - Passing a uniquely owned, pre-sized `Dict` through a closure loses uniqueness in `nightly-2026-09-05-b195f5b`, causing one allocation per insertion even though the reserved capacity is sufficient. Red target: `examples/builtins/dictInsertUniqueness.roc`.
