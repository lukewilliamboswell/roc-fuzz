Bugs found in Roc via fuzzing:
 ...
 - The same overwrite also allocated once per call when the keys and values were refcounted (`Dict(Str, List(U8))` with keys padded past the 24-byte inline threshold), scaling linearly with the number of overwrites, which the primitive-case fix did not cover. Fixed by roc `92a663ecb8` ("Preserve Dict uniqueness for refcounted overwrites"); `trophy-case/repros/dictInsertOverwriteRefcounted.roc` now passes.
