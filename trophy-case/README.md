Bugs found in Roc via fuzzing:
 ...
 - `roc check` panics with `postcheck invariant violated: erroneous checked type reached Monotype instantiation` instead of reporting a type error, when a `Try`-returning builtin's result is assigned to a `var` of the unwrapped type (e.g. `$l = List.set($l, 0, 5)`). Repro: `trophy-case/repros/postcheck_panic_on_type_error.roc`
 - `Json.to_str` on a `U64` costs one allocation even when the rendered digits fit in a small (inline) string, suggesting the encoder round-trips through a growable heap buffer
