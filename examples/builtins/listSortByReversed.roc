app [target] { pf: platform "https://github.com/lukewilliamboswell/roc-fuzz/releases/download/0.4.0-rc1/9k2cfuAWoBfcRBRiVbriXFf1dHktoRBbieifYN7NmTHc.tar.zst", roc: "nightly-2026-09-15-fe09c42" }
...
## `items` is uniquely owned here, so `List.sort_by_reversed` must sort in
...
## buffer plus the projected-key buffer `sort_by_reversed` builds, and a
## degenerate small/empty-list path); the count stays flat across list
## lengths, never scaling with the number of elements.
test! : Input => Fuzz.Outcome
test! = |input| {
	items = decorate(input)
	sorted = Fuzz.expect_allocs_at_most!(5, |{}| List.sort_by_reversed(items, |item| item.key))
